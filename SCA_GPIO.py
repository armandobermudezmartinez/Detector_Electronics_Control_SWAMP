...
class SCA_GPIO:
    ...
    def _enable_gpio(self, enableGPIO=1):
        ...
        self.transactor.write(CTRL["CHANNEL_CTRL"],
                              CTRL["W_CRB"], mask=mask, data=data, comment='Enabling Pins')

    def _read_enable_gpio(self):
        self.transactor.write(
            CTRL["CHANNEL_CTRL"], CTRL["R_CRB"], comment='Reading Enable Pins register')

    def _set_gpio_mode(self, pin, mode):
        self._mode |= mode << pin
        self.transactor.write(
            GPIO["CHANNEL"], GPIO["W_DIRECTION"], mask=self._mode, data=self._mode, comment='Setting Pin Mode')

    def _get_gpio_mode(self):
        self.transactor.write(
            GPIO["CHANNEL"], GPIO["R_DIRECTION"], comment='Reading Pin Mode')
