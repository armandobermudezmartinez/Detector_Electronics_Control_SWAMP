    ...
    def _enable_i2c(self, index):
        ...
        if (index_bit & INPUTMASK_CRB):
            ...
            self.transactor.write(CTRL["CHANNEL_CTRL"],
                                  CTRL["W_CRB"], mask=mask, data=data, comment=f'Enabling I2C {index}')
        ...
        if (index_bit & INPUTMASK_CRC):
            ...
            data = from_8bit_to_32bit(d3)
            self.transactor.write(CTRL["CHANNEL_CTRL"],
                                  CTRL["W_CRC"], mask=mask, data=data, comment=f'Enabling I2C {index}')
        ...
        if (index_bit & INPUTMASK_CRD):
            ...
            data = from_8bit_to_32bit(d3)
            self.transactor.write(CTRL["CHANNEL_CTRL"],
                                  CTRL["W_CRD"], mask=mask, data=data, comment=f'Enabling I2C {index}')

    def _read_enable_i2c(self):
        self.transactor.write(
            CTRL["CHANNEL_CTRL"], CTRL["R_CRB"], comment='Reading I2C enable register')

        self.transactor.write(
            CTRL["CHANNEL_CTRL"], CTRL["R_CRC"], comment='Reading I2C enable register')

        self.transactor.write(
            CTRL["CHANNEL_CTRL"], CTRL["R_CRD"], comment='Reading I2C enable register')

    def _set_i2c_speed(self, index, speed):
        channel = I2C["CHANNEL_MAP"][index]
        if speed not in [100, 200, 400, 1000]:
            raise Exception("Bus speed not among permissible values")
        speeds = {
            100: 0b00,
            200: 0b01,
            400: 0b10,
        ...
        self.transactor.write(
            channel, I2C["W_CTRL_REG"], mask=mask, data=data, comment=f'Setting I2C speed to: {speed}')

    def _read_i2c_speed(self, index):
        channel = I2C["CHANNEL_MAP"][index]
        self.transactor.write(
            channel, I2C["R_CTRL_REG"], comment='Reading I2C speed')
...
class I2C_Master(SCA_I2C):
    ...
    def set_speed(self, new_speed):
        self._set_i2c_speed(self.index, new_speed)
